...
exports.extractDriveFrames = onRequest(
  { cors: true, timeoutSeconds: 60, memory: "1GiB" },
  async (req, res) => {
    ...
    try {
      ...
      const tmpRoot = await fs.mkdtemp(
        path.join(os.tmpdir(), "signet-drive-frames-")
      );
      const results = [];
      const diagnostics = {
      ...
      const tryUrl = async (inputUrl) => {
        ...
        for (const ts of timestamps) {
          ...
          if (cmd.status !== 0) {
            ...
            if (!sampleError) {
              const timeoutErr =
                cmd.error && cmd.error.code === "ETIMEDOUT"
                  ? "ffmpeg_timeout"
                  : "";
              sampleError =
                timeoutErr ||
                String(cmd.stderr || cmd.stdout || "")
                  .split("\n")
                  .map((l) => l.trim())
                  .filter(Boolean)[0] || "ffmpeg_error";
            }
            continue;
          }
          try {
            ...
            current.push({
              timestamp: ts,
              imageUrl: `data:image/jpeg;base64,${bin.toString("base64")}`,
            });
          } catch (e) {
            failed.push(ts);
          }
      ...
      diagnostics.missingTimestamps = timestamps.filter(
        (ts) => !results.some((r) => r.timestamp === ts)
      );

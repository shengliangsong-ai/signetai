...
export const Notebook: React.FC<NotebookProps> = ({ onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[200]">
      <div className="bg-[var(--bg-standard)] w-full max-w-3xl h-auto max-h-[85vh] rounded-xl shadow-2xl flex flex-col">
        ...
        <div className="p-6 flex-1 overflow-y-auto">
          <div className="space-y-6">
            {demoSteps.map((step, index) => (
              <div key={index} className={`p-5 rounded-lg transition-all duration-500 ${currentStep === index && isRunning ? 'bg-[var(--bg-subtle)] border-2 border-[var(--trust-blue)] shadow-xl' : 'bg-[var(--code-bg)] border-2 border-transparent opacity-70'}`}>
                <h3 className="font-bold text-md text-[var(--text-header)]">{step.title}</h3>
                
                {currentStep === index && isRunning && (
                    <div className="mt-4 mb-2 animate-fade-in-fast rounded-lg overflow-hidden">{step.visual}</div>
                )}
                
                <p className="text-sm text-[var(--text-body)]">{step.transcript}</p>
              </div>
            ))}
          </div>
        </div>

...
*   **Dynamic 3D Avatar & Designer Lab:** The Live Agent features a responsive 3D human-like avatar that reacts to the conversation. It includes real-time lip-syncing when the agent speaks, natural eye movements (blinking), head bobbing, and automatically switches its gender appearance based on the selected voice. Users can fully customize their digital identity in the **Avatar Designer Lab**, which includes a "Generate from Photo" feature powered by Gemini 3.1 Pro/Flash to instantly map their real-world facial features to the 3D avatar.
